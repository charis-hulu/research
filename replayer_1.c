...
//    char time_b = * ( (char*) p2 );
 ...
 //   char time_a_cp[10];
//    char time_b_cp[10];
 //   memcpy(time_b_cp, time_b , 9);
  //  memcpy(time_b_cp, time_b , 9);
    
  //  float float_time_a = atof(time_a_cp);
   // float float_time_b = atof(time_b_cp);
   // if ( float_time_a == float_time_b ) return -1;
   // else if ( float_time_a < float_time_b ) return 0;
   // else return 1;
//}
...
int64_t read_trace(char ***req, char **tracefiles)
{
    ...
    FILE *trace_1 = fopen(tracefiles[0], "r");
    FILE *trace_2 = fopen(tracefiles[1], "r");
    ...
    while (fgets(line, sizeof(line), trace_1) != NULL)
    {
    ...
    while (fgets(line, sizeof(line), trace_2) != NULL)
    {
        line[strlen(line) - 1] = '\0';
        if (((*req)[i] = malloc((strlen(line) + 1) * sizeof(char))) == NULL)
        {
    ...
    //printf("--> %d\n", nr_lines);
    //qsort((*req), nr_lines, sizeof(*req), comparator);
    //qsort((*req), nr_lines, sizeof(char**), comparator);
...
void *perform_io(void *arg)
{
	...
	//printf("Thread %d -- [%ld] io_limit__ = %d\n", thread, cur_idx, io_limit__);
	//printf("Thread %d -- [%ld] size__     = %d\n", thread, cur_idx, size__);
	//printf("Thread %d -- [%ld] offset__   = %ld\n", thread, cur_idx, oft[cur_idx]);
	//printf("\n");
        ...
        if (respecttime == 1)
        {
            ...
            if (elapsedtime < (int64_t)(timestamp[cur_idx] * 1000))
            {
                ...
                sleep_time = (useconds_t)(timestamp[cur_idx] * 1000) - elapsedtime; //microsecond
                //printf("Thread %d -- [%ld] elapsedtime  = %ld microseconds\n", thread, cur_idx, elapsedtime);
                //printf("Thread %d -- [%ld] timestamp    = %f miliseconds\n", thread, cur_idx, timestamp[cur_idx]);
                //printf("Thread %d -- [%ld] sleep_time   = %d microseconds\n", thread, cur_idx, sleep_time);
            	 //printf("\n");
        ...
        while (size__ > 0)
        {
            ...
            i++;
	    //printf("Thread %d -- [%ld] size__     = %d\n", thread, cur_idx, size__);
    	    //printf("Thread %d -- [%ld] ret        = %d\n", thread, cur_idx, ret);
    	    
            if (ret <= 0)
            {
                //printf("[%ld] Cannot %s size %d to offset %lu! ret=%d, errno=%d\n", cur_idx,
...
void do_replay(void)
{
    ...
    for (t = 0; t < nr_workers; t++)
    {
        ...
        *a = t;
        assert(pthread_create(&tid[t], NULL, perform_io, a) == 0);
    }
...
int main(int argc, char **argv)
{
    ...
    char tracefile_1[256], tracefile_2[256];
    char logfile[256];
    char **request; 
    ...
    else
    {
        ...
        printf("Disk ==> %s\n", device);
        printf("%s\n",argv[2]);
        sprintf(tracefile_1, "%s", argv[2]);
        printf("Trace 1 ==> %s\n", tracefile_1);
        sprintf(tracefile_2, "%s", argv[3]);
        printf("Trace 2 ==> %s\n", tracefile_2);
        sprintf(logfile, "%s", argv[4]);
        printf("Logfile ==> %s\n", logfile);
    }
    ...
    if ((tracefiles = malloc(2 * sizeof(char *))) == NULL)
    {
        fprintf(stderr, "memory allocation error (%d)!\n", __LINE__);
        exit(1);
    }
    ...
    tracefiles[0] = tracefile_1;
    tracefiles[1] = tracefile_2;
    // read the trace before everything else
    nr_tt_ios = read_trace(&request, tracefiles);
    //printf("nr_tt_ios %d\n", nr_tt_ios);
    //for(int j = 0; j < nr_tt_ios; j++){
    //	printf("[%d] %s\n", j, request[j]);
    //}
    //printf("%s\n", *request);
    //printf("%d\n", **request);
